...
import com.github.vkuzel.orm_frameworks_demo.openjpa.repository.OperatorRepository;
import com.github.vkuzel.orm_frameworks_demo.openjpa.repository.PlaneRepository;
import com.github.vkuzel.orm_frameworks_demo.openjpa.repository.RegistrationRepository;
...
@Service
public class OpenJpaAirlinesService implements AirlinesService {
    ...
    @Autowired
    public OpenJpaAirlinesService(PlaneRepository planeRepository, OperatorRepository operatorRepository, RegistrationRepository registrationRepository) {
    ...
    private Page<OperatorDetail> findAllOperatorsOrderByName(String languageCode, Pageable pageable, boolean ascending) {
        ...
        if (ascending) {
            operators = operatorRepository.findAllOrderByNameAsc(languageCode, pageable.getPageSize(), pageable.getOffset());
        } else {
            operators = operatorRepository.findAllOrderByNameDesc(languageCode, pageable.getPageSize(), pageable.getOffset());
        }
        return new PageImpl<>(
